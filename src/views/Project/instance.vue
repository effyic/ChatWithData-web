    ...
    <div class="create-instance">
      <div class="header">
        ...
        <div class="header-actions">
          <el-button class="qa-btn" link @click="toQuestion">
                ...
                <div class="table-items">
                  ...
                  >
                    ...
                    <el-button 
                      ...
                      link 
                      class="add-info-btn"
                      @click.stop="showAddInfoDialog(table)"
                    >
            ...
            <Transition name="slide">
              <div class="field-content" v-if="currentTable">
                <div class="field-list">
                    <div class="field-list-header" style="margin-bottom: 10px;color:#073B8E;font-weight: 500;">
    表描述信息：{{ currentTable?.comment || '-' }}
  </div>
      ...
      >
        <el-form
          ...
          inline
        >
        <el-form-item label="数据库" prop="dsn_type">
            <el-select v-model="formData.dsn_type" placeholder="请选择数据库类型">
...
const databaseId = ref()
  ...
  const getDatabaseList = async () => {
    ...
    try {
      ...
      // 将数据库名称转换为对象数组，初始时 id 为 null
      databaseList.value = databaseRes.data.map(name => ({
        name,
        id: undefined
      }))
      
      // 获取已选中的数据库列表
      const selectedDBs = usedRes.data || []
      
      // 更新选中的数据库名称列表
      selectedDatabases.value = selectedDBs.map(item => item.name)
      
      // 更新 databaseList 中匹配项的 id
      databaseList.value.forEach(db => {
        const matchedDB = selectedDBs.find(selected => selected.name === db.name)
        if (matchedDB) {
          db.id = matchedDB.id
        }
      })
  ...
  // 显示数据库选择弹窗
  const showDatabaseDialog = async (id) => {
    currentInstanceId.value = id //当前instance id
    tableList.value = [] //表清空
    currentTable.value = null
    databaseDialogVisible.value = true
    // 获取数据库列表
    loading.value = true
    defaultDialogSelected()
  }


//   处理弹窗选中显示逻辑
  const defaultDialogSelected = async () => {
    try {
      const [databaseRes, usedRes] = await Promise.all([
        projectStore.database(currentInstanceId.value),
        projectStore.used(currentInstanceId.value)
      ...
      // 找到第一个有 id 的数据库作为当前显示的数据库
      const firstDBWithId = databaseList.value.find(db => db.id !== undefined)
      if (firstDBWithId) {
        currentDatabase.value = firstDBWithId.name
        databaseId.value = firstDBWithId.id
        // 获取该数据库的表列表
        getTableList(firstDBWithId.id)
      } else {
        // 如果没有有 id 的数据库，清空选中状态
  ...
  const selectTable = (table) => {
    currentTable.value = table //当前选中的表
    tableId.value = table.id //当前选中的表id
    getFieldsList() //获取字段列表
  }
  
  const getFieldsList = () => { 
    fieldLoading.value = true
    projectStore.fields(currentInstanceId.value,databaseId.value,tableId.value).then((res) => {
  ...
  const handleCheckboxChange = (checked, db) => {
    if (!checked) {
        ...
        handleSaveDatabases(db.name)
    }
  }

  const handleDatabaseClick = (db) => {
    //   selectedDatabases.value.push(db.name)
    //   handleSaveDatabases(db.name)
       currentDatabase.value = db.name
        if (!selectedDatabases.value.includes(db.name)) {
            selectedDatabases.value = [...selectedDatabases.value, db.name]
        }
        handleSaveDatabases(db.name)
  }

   // 保存选中的数据库
   const handleSaveDatabases = (dbName) => {
    projectStore.update(currentInstanceId.value,{
        name:Array.from(new Set(selectedDatabases.value))
    }).then((res) => {
        if(res.data) {
            // 更新 databaseList 中的 id
            databaseList.value.forEach(db => {
                const matchedDB = res.data.find(item => item.name === db.name)
                if (matchedDB) {
                    db.id = matchedDB.id
                    // 设置当前库id
                    if(db.name === dbName) {
                        databaseId.value = matchedDB.id
                        selectDatabase(dbName, matchedDB.id)
                    }
                }
            })
        } else {
  ...
  // 库查表
  const selectDatabase = (dbName, id) => {
    if (!dbName) return
  ...
  const getTableList = (id) => {
    ...
    projectStore.tables(currentInstanceId.value, id).then((res) => {
        nextTick(() => {
            tableList.value = res.data
            // 如果有当前选中的表，重新选中它以更新描述信息
            if (currentTable.value) {
                const table = res.data.find(t => t.id === currentTable.value.id)
                if (table) {
                    selectTable(table)
                }
            }
  ...
  const handleCreate = async () => {
    ...
    await formRef.value.validate((valid, fields) => {
      if (valid) {
        ...
        // 用户:密码@tcp(ip:3306)
        const databaseType = formData.dsn_type === 'tdengine' ? 'taos' : 'dm'
        // const url =`${username}:${password}@tcp(${host}:${port})`
        let url;
        if(formData.dsn_type === 'mysql'){
            url = `${username}:${password}@tcp(${host}:${port})`
        }else{
            url = `${databaseType}://${username}:${password}@${host}:${port}`
        }
         switch(dialogType.value){  
          case 'create':
            projectStore.createDsn(projectStore.projectId,{
                name:formData.name,
                dsn_type:formData.dsn_type,
                url
            }).then(() => {
                ElMessage.success('保存成功')
  ...
  // 显示补充信息弹窗
  const showAddInfoDialog = (table) => {
    // 阻止事件冒泡，避免触发表格的点击事件
    event?.stopPropagation()
    // 更新当前选中的表
    currentTable.value = table
    tableId.value = table?.id
    infoForm.tableDescription = table?.comment || ''
    addInfoDialogVisible.value = true
  }
  
  // 保存补充信息
  const handleSaveInfo = () => {
    projectStore.updateComment(tableId.value, {
      comment: infoForm.tableDescription
    }).then((res) => {
      ElMessage.success('保存成功')
      addInfoDialogVisible.value = false
      // 重新获取表列表以更新备注
      getTableList(res.data.database_id)
    }).catch(() => {
  ...
  const testConnection = () => {
    const {host,port,username,password,dsn_type} = formData
    const databaseType = dsn_type === 'tdengine' ? 'taos' : 'dm'
    ...
    projectStore.test(url,dsn_type).then((res) => {
  ...
  const formatUrl = (url, type) => {
    ...
    // 检查是否是 MySQL 格式 root:root@tcp(192.168.0.160:3306)
    if (url.includes('@tcp(')) {
      const [userInfo, hostInfo] = url.split('@tcp(')
      const [username, password] = userInfo.split(':')
      const [host, port] = hostInfo.slice(0, -1).split(':') // slice(0, -1) 移除最后的 )
  ...
  $btn-color: #007dff;
  $title-color: #073B8E;
$secoundTitle-color:#3962A5;
// #F5F7FA #F9FBFD
$database-bgcolor:#F6F8FB;
$active-bgcolor:#EBF0F7;
$table-bgcolor:#F3F6FA;
// $description-color:#073B8E;
  ...
  :deep(.database-dialog) {
    ...
    .database-content {
      ...
      .table-list {
        ...
        background-color: $database-bgcolor;
